"""Module 10 runner: MCP client + Redis business banking use case."""
...
def build_runner() -> RuntimeBundle:
    ...
    app_name = f"{settings.app_name}_module10_mcp_client"
    runner = Runner(
...
def _extract_last_text(events: Iterable) -> str:
    ...
    for event in events:
        ...
        if text:
            last_text = text
    if not last_text:
        raise RuntimeError("The Module 10 MCP run completed without final text output.")
    return last_text
...
def run_prompt(prompt: str, user_id: str = "demo-user", session_id: str | None = None) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "Module 10 MCP run failed. Check MODULE10_MCP_COMMAND / MODULE10_MCP_ARGS and Redis MCP availability. "
...
async def stream_prompt(
    ...
    try:
        ...
        raise RuntimeError(
            "Module 10 MCP stream failed. Check MODULE10_MCP_COMMAND / MODULE10_MCP_ARGS and Redis MCP availability. "
            f"Original error: {exc}"
        ) from exc
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the Module 10 MCP client lesson (Redis business banking use case).",
    )
    parser.add_argument(
        "prompt",
        nargs="?",
        default="CUST-1001: review current business banking profile and store summary in Redis memory.",
        help="Prompt to send to the Module 10 MCP client agent.",
    )
    args = parser.parse_args()

    async def _run() -> None:
        print("MCP Client Agent: ", end="", flush=True)
